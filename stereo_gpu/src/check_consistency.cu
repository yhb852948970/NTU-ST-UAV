...
namespace {
	...
	__global__ void check_consistency_kernel(uint16_t* d_leftDisp, const uint16_t* d_rightDisp, const uint8_t* d_left, int width, int height)  {
...
namespace sgm {
	namespace details {
		...
		void check_consistency(uint16_t* d_left_disp, const uint16_t* d_right_disp, const void* d_src_left, int width, int height) {
			...
			check_consistency_kernel<< < blocks, threads >> > (d_left_disp, d_right_disp, (uint8_t*)d_src_left, width, height);
